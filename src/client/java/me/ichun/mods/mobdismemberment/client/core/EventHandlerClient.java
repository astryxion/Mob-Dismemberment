...
public class EventHandlerClient {
    ...
    public void register() {
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> onClientConnection());
        ClientTickEvents.END_CLIENT_TICK.register(this::worldTick);
    }
    ...
    public void worldTick(Minecraft mc) {
        if (mc.level != null) {
            ...
            if (!mc.isPaused()) {
                ...
                // Note: We don't manually tick - addEntity registers the gib with the client world's entity tick list
    ...
    public boolean dismember(ClientLevel world, LivingEntity living, Entity explo) {
        ...
        if (Config.getBlood()) {
            int bloodParticles;
            float explosionSprayMult;
            if (explo == null) {
                bloodParticles = Config.getBloodCount();
                explosionSprayMult = 0F;
            } else if (explo instanceof Creeper) {
                bloodParticles = Math.min(Config.getBloodCount() * 2, 96);
                explosionSprayMult = 6F;
            } else {
                bloodParticles = Math.min(Config.getBloodCount() * 10, 450);
                explosionSprayMult = 100F;
            }
            ...
            for (int k = 0; k < bloodParticles; k++) {
                ...
                if (explosionSprayMult > 0F) {
                    var4 *= explosionSprayMult;
                }
                ...
                world.addParticle(
                        MobDismembermentClient.BLOOD_PARTICLE,
                        living.getX(),
     ...
     * Uses {@link ClientLevel#addEntity} so the entity is tracked and ticked like other client entities.
     * Also adds to our activeGibs list for cleanup tracking.
     */
